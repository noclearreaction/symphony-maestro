...
The `rubato:anchor` JSON SHALL accept an optional top-level `options` array of `{"key", "value"}` objects. Parsers SHALL scan all entries for recognised `key` values and ignore entries with unknown keys. Absence of `options` SHALL be treated as equivalent to an empty array.

#### Scenario: Anchor with options array
- **WHEN** the anchor JSON contains `"options":[{"key":"max_age","value":50}]`
- **THEN** `Block.Options` SHALL contain one entry with key `max_age` and value `50`
...
#### Scenario: Unknown option keys are ignored
- **WHEN** an options entry has an unrecognised `key` value
...
The anchor package SHALL expose a `MaxAge() int` method on `Block` that scans `Options` for an entry where `key == "max_age"` and returns its `value` as int, defaulting to 100 when not found.
