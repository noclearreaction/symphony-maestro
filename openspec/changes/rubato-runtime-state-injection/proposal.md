...
- Add cache-stable plugin-instruction augmentation in `messages[0]`: each declared plugin contributes a canonical usage/integration instruction block once per session, and identical anchor/plugin inputs must produce byte-identical guidance across sessions.
- Start MVP with a `git_status` plugin focused on branch and hygiene signals (branch name, ahead/behind, committed, staged, untracked counts).
...
### New Capabilities
- `rubato-runtime-injection`: Marker/anchor driven runtime state injection for AI requests using plugin-selected context blocks.
